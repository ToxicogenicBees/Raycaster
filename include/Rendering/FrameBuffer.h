...
class FrameBuffer {
    ...
    public:
        ...
        static void resize(uint16_t size_x, uint16_t size_y) { _buffer.resize(size_x, size_y); }
        static void resize(const Point<uint16_t>& size) { _buffer.resize(size); }
        static void fill(const Color& color) { _buffer.fill(color); }
        static uint16_t sizeX() { return _buffer.sizeX(); }
        static uint16_t sizeY() { return _buffer.sizeY(); }
};
